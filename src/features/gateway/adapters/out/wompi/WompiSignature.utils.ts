...
export function resolvePropertyValues(payload: WompiEventPayload, properties: string[]): string[] {
...
export function computeEventChecksum(payload: WompiEventPayload, eventsKey: string): string {
  const values = resolvePropertyValues(payload, payload.signature?.properties ?? []);
