...
import { computeEventChecksum, WOMPI_BASE_URLS } from "./WompiSignature.utils.js";
...
export class WompiAdapter implements IWompiPort {
  ...
  async createPaymentLink(input: CreateWompiLinkInput, privateKey: string): Promise<WompiLinkData> {
    ...
    const res = await this.httpClient.post<unknown>(`${await this.baseUrl()}/payment_links`, body, {
  ...
  async getLinkTransactions(linkId: string, privateKey: string): Promise<WompiEventTransaction[]> {
    const auth = { headers: { Authorization: `Bearer ${privateKey}` } };
    try {
