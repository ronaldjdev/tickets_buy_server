...
} from "@/features/gateway/application/use-cases/VerifyGatewayTransaction.uc.js";
...
export class GatewayController {
  ...
  getIntent = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const intent = await this.getPublicIntent.execute(req.params.reference as string);
  ...
  syncIntent = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const input: VerifyGatewayTransactionInput = {
        ...
        transactionId: (req.query.transactionId as string | undefined) || undefined
      };
