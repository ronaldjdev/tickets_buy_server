...
import type { IWompiPort, WompiEventTransaction } from "@/shared/port/IWompi.port.js";
...
export class VerifyGatewayTransaction {
  ...
  async execute(input: VerifyGatewayTransactionInput): Promise<VerifyGatewayTransactionResult> {
    if (!input?.reference) throw new UseCaseError("La referencia es obligatoria.");

    const intent = await this.intentRepo.findByReference(input.reference);
    if (!intent) throw new UseCaseError("Cobro no encontrado.");
    if (intent.gateway !== "wompi") throw new UseCaseError("El cobro no pertenece a Wompi.");
    ...
    if (!transaction || !this.belongsToIntent(intent, transaction)) return unchanged;
  ...
  private async findTransaction(
    ...
    const transactions = await this.wompiPort.getLinkTransactions(intent.linkId, privateKey);
    return transactions[0] ?? null;
  }

  private belongsToIntent(intent: GatewayIntent, transaction: WompiEventTransaction): boolean {
    const matchesReference = Boolean(
      transaction.reference && transaction.reference === intent.reference
    );
    const matchesLink = Boolean(intent.linkId && transaction.payment_link_id === intent.linkId);
