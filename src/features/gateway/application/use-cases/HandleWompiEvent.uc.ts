...
import type { IWompiPort, WompiEventPayload } from "@/shared/port/IWompi.port.js";
...
export class HandleWompiEvent {
  ...
  async execute(payload: WompiEventPayload): Promise<HandleWompiEventResult> {
    ...
    const intent =
      (transaction.reference && (await this.intentRepo.findByReference(transaction.reference))) ||
      (transaction.payment_link_id &&
        (await this.intentRepo.findByLinkId(transaction.payment_link_id)));

    if (!intent) {
      this.logger.warn("[Wompi] Transacción sin cobro asociado", { id: transaction.id });
      return { handled: false, reason: "intent_not_found" };
    }
    if (intent.status === "pagada") {
      return { handled: false, reason: "already_processed", intentStatus: intent.status };
    }
