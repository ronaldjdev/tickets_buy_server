...
export interface IWompiPort {
  createPaymentLink(input: CreateWompiLinkInput, privateKey: string): Promise<WompiLinkData>;
  verifyEventChecksum(payload: WompiEventPayload, eventsKey: string): boolean;
  getTransaction(transactionId: string, privateKey: string): Promise<WompiEventTransaction | null>;
  getLinkTransactions(linkId: string, privateKey: string): Promise<WompiEventTransaction[]>;
}
