...
export async function connectDB(uri: string) {
  try {
    ...
    if (!mongoose.connection.db) {
      throw new Error("La conexión a la base de datos no está establecida completamente.");
    }
    ...
    logger.error("❌ Error conectando a MongoDB:", { operation: "connectDB", error });
    throw error;
  }
}
...
process.on("SIGINT", async () => {
  try {
    ...
    logger.error("Error cerrando conexiones:", { operation: "connectDB", error: err });
    process.exit(1);
  }
});
