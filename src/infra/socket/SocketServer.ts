...
export class SocketServer implements ISocketServer {
  ...
  init(server: HttpServer, clientUrl: string) {
    ...
    this.io = new SocketIOServer(server, {
      cors: {
        origin: process.env.NODE_ENV === "production" ? [clientUrl, "http://localhost:3000"] : true,
        credentials: true
      },
    ...
    this.io.on("connection", (socket) => {
      ...
      socket.on("join_conversation", ({ conversationId }) => {
        ...
        logger.info(`[Socket] ${socket.id} se unió a conversation:${conversationId}`);
      });

      socket.on("leave_conversation", ({ conversationId }) => {
        if (!conversationId) return;
        socket.leave(`conversation:${conversationId}`);
        logger.info(`[Socket] ${socket.id} salió de conversation:${conversationId}`);
      });
